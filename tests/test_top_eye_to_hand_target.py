...
class TopEyeToHandTargetTest(unittest.TestCase):
    def test_board_contract_and_marker_ids(self):
        self.assertAlmostEqual(MODULE.BOARD_SIDE_M, 0.045)
        ...
        self.assertEqual(board.ids.reshape(-1).tolist(), [0, 1, 2, 3])

    def test_generated_board_detects_all_four_ids(self):
        board_image = MODULE.draw_board(1800)
        ...
        self.assertEqual(sorted(ids.reshape(-1).tolist()), [0, 1, 2, 3])
