...
| MOT-004 | 단계 8 | Pick pregrasp 연속 buffered Action | 충돌 검사 경로를 20 ms dense Action으로 변환하고 fresh-start·terminal·post-settle·physical DISABLE 통과 | 부분 통과 | [Motion-11 plan-only](test-results/2026-08-04-motion11-buffered-pick-pregrasp-plan-only.md)의 첫 9.1초 실기는 물리 경로를 따라 움직였으나 Shoulder/Wrist Flex 추종 부족으로 post-settle ABORTED가 됐다. 자동 재시도 없이 실측 약 60 raw/s를 보수적 50 raw/s로 반영한 43초·2151 sample 후보를 생성했다. fresh anchor 재생성과 물리 재검증 전이며 `motion_authorized=false`다. |
