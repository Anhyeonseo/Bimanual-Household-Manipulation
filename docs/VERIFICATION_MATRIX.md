...
| MOT-003 | 단계 8 | 왼팔 multi-point/buffered trajectory | 시간축·queue·cancel·soft-abort·SAFE_STOP 실기 통과, 불필요한 정착 정지 제거 | 부분 통과 | [Motion-1](checklists/MOTION_BUFFERED_TRAJECTORY_CONTRACT.md)부터 [Motion-8](checklists/MOTION_G474_BUFFERED_PHYSICAL_ROUTE.md)까지 구현하고 `0x00022100` Motion-9 소형 다중 관절 Action 왕복을 실기 통과했다. [Motion-10 q0 왕복](test-results/2026-08-04-motion10-buffered-q0-roundtrip-plan-only.md)은 실제 왕복을 완료했지만 전 구간 떨림과 post-settle abort로 부분 실패했다. dense quintic·position-only settle 로컬 수정 뒤 축별 추종과 q0 재시험이 남아 있다. |
