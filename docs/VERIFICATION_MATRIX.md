...
| VIS-003 | 단계 8 | 시연 환경 강건 펜 검출 | 카메라 각도·높이·물체 Z 고정, 배경·조명·반사만 다른 조건에서 물체 1개와 위치/yaw·miss·false positive 기준 충족 | 부분 통과 | [기준선 계약 결과](test-results/2026-08-02-top-pen-detection-baseline-contract.md), [수집 체크리스트](checklists/STAGE8_TOP_PEN_DETECTION_BASELINE.md). 재배치 frame의 `detected 3 (ignored 1 fully outside)`를 재현했으며 최소 18장 dataset·새 backend 비교는 미실행 |
