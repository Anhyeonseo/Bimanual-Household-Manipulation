...
**후속 계획**: `docs/PLAN_TOWEL_FOLDING_PERCEPTION.md`. 이 트랙(W0–W4)이
끝나면서 손목 카메라의 역할이 원 계획의 "미세 위치 보정" 에서
**"손끝 국소 검증 + 국소 기하 측정"** 으로 바뀌었다. 근거는 W4 의 실측
(보정 대역이 `[8, 30] mm` 라 정상 파지를 개선하지 못함)이며, 최종 목표인
수건 접기 기준으로 재정리한 것이 그 문서다.
...
## W4 — bounded visual correction — **판정 계층 완료, 검출기·물리 미착수**

### W4 에서 실측으로 드러난 것 — 먼저 읽을 것

**절대 Cartesian 보정으로는 "마지막 수 cm 미세 보정" 이 이 정확도에서
성립하지 않는다.** 판정 계층을 만든 뒤, W3 세션의 **움직이지 않은** 표적을
10개 자세에서 그대로 이 판정기에 넣어 오탐률을 실측했다. 표적이 가만히
있었으므로 `apply` 는 전부 헛보정이다.

| 보정 하한 | 헛보정(오탐) |
|---|---|
| `4 mm` (XY 평균 오차에서 유도 — **틀렸다**) | **4/10** |
| `5 mm` | 3/10 |
| `6 mm` | 1/10 |
| `7 mm` | 0/10 |

관측된 헛보정 크기(mm): `0.95 1.55 1.66 1.75 2.08 3.00 4.67 5.64 5.99 6.42`.
하한은 **평균이 아니라 최악값**에서 잡아야 하며, `7 mm` 에서 오탐이 사라져
`8 mm` 를 여유로 채택했다.

**따라서 보정 대역은 `[8, 30] mm` 이고, 이 계층은 큰 오차를 잡는 장치이지
미세 조정 장치가 아니다.** C2 에서 파지는 잔차 `10.17 mm` 에서 성립했으므로
정상 회차를 이 계층이 더 좋게 만들지는 못한다. 잡을 수 있는 것은 "상단
카메라가 말한 위치와 물체가 실제로 있는 위치가 유의하게 다르다" 는 경우다.

**왜 더 못 줄이는가, 그리고 무엇이 답인가.**

- 흩어짐이 자세마다 **체계적**이고 무작위가 아니다. 각 capture 가 이미
  20 frame median 이라 같은 자세에서 frame 을 더 모아도 줄지 않는다.
- 이 오차에는 카메라 보정 오차만 있는 게 아니라 **팔 자신의 FK 오차(처짐·
  백래시)가 섞여 있다.** C2 는 SHOULDER 처짐만 `7.42 mm` 로 실측했고 여기
  `6.42 mm` 와 같은 크기다.
- **처짐이 들어오는 경로가 핵심이다.** 카메라는 그리퍼에 붙어 있으므로
  그리퍼 기준 **상대** 오차는 처짐과 무관하게 옳다. 처짐은 base 절대 좌표로
  올렸다 내리는 과정에서만 섞인다. 즉 `CAMERA_COMPUTE_ARCHITECTURE.md:61` 이
  병기한 **영상 기반 Visual Servo**(또는 그리퍼 상대량 보정)로 가면 이 한계
  아래로 내려갈 수 있다. 절대 좌표 재계획으로는 못 내려간다.

이것이 W4 의 다음 설계 결정이며, 아직 결정하지 않았다.

### 항목

- [x] `tools/wrist_visual_correction.py` — 판정 계층. `grasp_convergence.py`
      의 구조를 따른다(팔별 frozen policy, 순수 판정, 보내지 않음,
      fail-closed). 18 test
- [x] **Z 를 보정하지 않는다.** W3 축별 실측이 근거 —
      X `std 3.06 / max 6.41 mm`, Y `std 2.35 / max 5.95 mm`,
      **Z `std 5.65 / max 9.13 mm`**. 평면 표적 단안 깊이 관측성이 약해
      전체 잔차 `6.81 mm` 의 대부분이 Z 다. XY 만 보면 `평균 3.37 /
      최대 6.42 mm`. Z 는 작업대·물체 높이에서 이미 알므로 복원할 필요도
      없다 — `CAMERA_COMPUTE_ARCHITECTURE.md:44` 가 상단에 대해 선언한 같은
      규칙, 구현은 `top_shadow_target.yaml` 의
      `object.center_height_above_board_m`
- [x] 보정 대역을 실측 오탐률에서 유도 (위 표). 상한 `30 mm` 는
      `grasp_convergence.MAXIMUM_CORRECTION_M` 과 같은 값·같은 논리
- [x] fail-closed: frame 노후(`>0.2 s`), 신뢰도 미달(`<0.7`), 검출 0개 또는
      2개 이상, 비유한값, 보정 후 작업영역 이탈 → 전부 거부. 거부 시 좌표를
      아예 산출하지 않는다. `hold` 는 조용한 포기가 아니라 측정값과 이유를
      함께 보고한다
- [x] **층을 섞지 않는다.** 이 모듈은 `grasp_convergence` 를 건드리지 않고
      **새 목표**만 낸다. 목표 갱신은 재계획으로만 간다
- [x] 헛보정 실측값을 회귀로 고정 — 10개 값 전부 `hold` 로 억제되고
      `15 mm` 는 통과함을 시험이 검사한다
- [ ] 손목 검출: 펜 중심·yaw 를 **카메라 좌표 상대 오차**로 낸다. ROI + 작은
      검출기 (`CAMERA_COMPUTE_ARCHITECTURE.md:60`). 상단 OBB 번들은 `top_board`
      도메인 전용이라 그대로 쓸 수 없다. **미착수 — W4 의 최대 미지수**
- [ ] 주입 지점. 권장은 Cartesian 재계획 — 보정된 x/y 로
      `ros_moveit_plan_grasp.py` 재호출. **위 발견을 감안해 절대 재계획으로
      갈지 Visual Servo 로 갈지 먼저 결정해야 한다**
