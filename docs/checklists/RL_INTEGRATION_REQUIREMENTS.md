...
- [ ] **`wrist_roll` 이 raw `1874..2219` ≈ `±15°` 뿐이다.**
      ...
      **2026-08-09 재검토**: 이 `±15°`는 기계적 한계가 아니라
      `2026-07-30-physical-range-revalidation.md`에서 wrist_roll을
      7 raw밖에 안 돌려보고 "기존 유지"로 넘어간 미검증 소프트웨어
      상속값이다. URDF 실제 한계는 `-4.3146..+1.2704 rad`(≈320°,
      `72.78934°`가 정확히 그 상한이다). 재측정(팔 지지 후 토크
      DISABLE, 양방향 끝까지) 전까지는 이 제약을 그대로 두되, 통과하면
      `Q_GRASP` 를 되살릴 수 있다. 상세: `docs/PLAN_TOWEL_FOLDING_PERCEPTION.md` 3절
