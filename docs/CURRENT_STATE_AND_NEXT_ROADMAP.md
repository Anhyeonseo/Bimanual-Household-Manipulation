...
범위와 근거는 [캔 → 쓰레기통 계획](PLAN_CAN_TO_BIN.md)에 있다.
2026-08-16 FK 실측으로 아래 3번의 "작업대에 수직"이 **달성 불가능**함이
확인되어 그 항목을 폐기했다. 접근축은 보정 영역에서 수직으로부터
14~77도 기울며, TCP 위치를 유지한 최소 기울기도 51.5도다.

1. 현재 Top YOLO-OBB 데이터셋과 holdout을 정리하고 캔 한 개의 장축 yaw를
   `modulo π`로 안정화한다. — **사실상 완료** (yaw p95 2.36°, holdout 확장만 남음)
2. 실측 캔 치수와 gripper open/close/contact 값을 bounded probe로 확정한다.
   — **미완료. 현재 최우선 블로커.** jaw gap을 mm로 잰 기록이 없다.
3. ~~그리퍼 접근축은 작업대에 수직으로 유지한다~~ — 폐기. 대신 매 계획에서
   접근축 기울기를 FK로 기록하고 상한 계약으로 검사한다. 닫힘 축을 캔 장축에
   90도로 맞추는 요구는 유지한다.
4. wrist-roll 등가 해를 operational limit 안에서 열거하고 현재 anchor에서
   회전량이 가장 작은 분기만 고정한다. 불필요한 180°/360° 회전은 금지한다.
   — solver는 `GraspYawKinematics.solve_wrist_roll_branches()`로 구현했다.
5. open-gripper 자세 확인→plan-only→validate-only→감독 1회 파지 순으로 승격한다.

핸드오버는 v1에서 제외한다. 오른팔 단독으로 쓰레기통까지 닿으므로
좌우 base 변환 실측이 임계 경로에서 빠진다.
