...
All the environments are registered in the `so101_grasp_rl` extension. They start
...
import so101_grasp_rl.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import so101_grasp_rl.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~so101_grasp_rl.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from so101_grasp_rl.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `so101_grasp_rl` extension."""
    ...
    if args_cli.show_presets:
        from so101_grasp_rl.utils.preset_cli import enumerate_task_presets
