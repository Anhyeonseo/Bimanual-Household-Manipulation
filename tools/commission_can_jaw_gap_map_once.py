...
**왜 새 도구인가.** `commission_can_gripper_probe_once.py` 는 target raw 를
`1948..2009` 로 막는다. 그것은 펜의 닫힘 대역이며 캔에 필요한 개방을 아예
명령할 수 없다. 그 도구의 상한을 늘리는 대신 캔 계측용을 따로 둔다 — 펜
경로가 쓰는 좁은 대역을 그대로 두어야 펜 회귀가 안 생긴다.
...
**개방 방향.** 펜 계획기가 open=raw 2048, close=raw 1948 을 쓰므로 raw 가
커지면 열린다. 이 도구는 그 가정을 믿지 않고 **첫 두 점의 실측으로 검증**한다.
...
그 값을 쓴다. 펜 실행기가 반복 실행에서 쓰는 경로와 같다.
"""
        ...
        for step_index, target_raw in enumerate(args.raw_steps):
            ...
            if len(samples) == 2:
                ...
                if raw_delta * gap_delta <= 0.0:
                    raise RuntimeError(
                        "opening direction is not what the pen path assumes: "
